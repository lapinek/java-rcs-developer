...
class SQLFilterVisitor implements FilterVisitor<String, Map<String, Object>> {
    ...
    String visitAndFilter(Map<String, Object> params, AndFilter filter) {
        return visitCompositeFilter(params, filter, "AND")
    }

    String visitOrFilter(Map<String, Object> params, OrFilter filter) {
        return visitCompositeFilter(params, filter, "OR")
    }

    def String visitCompositeFilter(Map<String, Object> params, final CompositeFilter filter, String operator) {
        StringBuilder sb = new StringBuilder()
        boolean first = true
        for (Filter subFilter : filter.filters) {
            if (first) {
                sb.append("( ").append(subFilter.accept(this, params)).append(" )")
                first = false
            } else {
                sb.append(" ").append(operator).append(" ( ").append(subFilter.accept(this, params)).append(" )")
            }
        }
        return sb.toString();
    }

    static def boolean isAlwaysTrue(AttributeFilter filter) {
        return null == filter.attribute || null == filter.attribute.value || filter.attribute.value.isEmpty()
    }

    static def String addParameter(final Map<String, Object> params, final Attribute attribute) {
